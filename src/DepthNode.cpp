...
#include <tp/qt/DepthNode.h>
#include <stdexcept>

namespace TP { namespace qt {
    using namespace TP::geom;

    DepthNode::DepthNode(uint32_t depth, const Extent<double>& extent) {
        $extent = extent;
        $depth = depth;
    ...
    const Extent<double>& DepthNode::getExtent(void) const {
        return $extent;
    }

    void DepthNode::query(const Extent<double>& extent, std::vector<const void*>& dataList, std::unordered_map<long, bool>& dataManifest) {
    ...
    void DepthNode::insert(const Point* point) {
        if ($nw != nullptr) {
    ...
    void DepthNode::subdivide(void) {
        ...
        $nw = new DepthNode($depth - 1, quadExtent);

        quadExtent.translate(tx, 0.0);
        $ne = new DepthNode($depth - 1, quadExtent);

        quadExtent.translate(0.0, ty);
        $se = new DepthNode($depth - 1, quadExtent);

        quadExtent.translate(-tx, 0.0);
        $sw = new DepthNode($depth - 1, quadExtent);
